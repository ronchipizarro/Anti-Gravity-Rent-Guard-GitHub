...
function TenantApplyContent() {
    ...
    const invitePropertyId = searchParams.get('property') || ''

    const [step, setStep] = useState(1)
    const [loading, setLoading] = useState(false)
    const [submitted, setSubmitted] = useState(false)
    const [appData, setAppData] = useState<any>(null) // eslint-disable-line @typescript-eslint/no-explicit-any
    const [inviteData, setInviteData] = useState<any>(null) // eslint-disable-line @typescript-eslint/no-explicit-any
    ...
    // Fetch invite data if invite token is present (broker invitation flow)
    useEffect(() => {
        if (inviteToken && invitePropertyId) {
            const fetchInvite = async () => {
                try {
                    const res = await fetch(`/api/broker/properties/public/${invitePropertyId}?token=${inviteToken}`)
                    if (res.ok) {
                        const data = await res.json()
                        setInviteData(data)
                        // Pre-fill renter name/email from invitation
                        if (data.invitation) {
                            setFormData(prev => ({
                                ...prev,
                                first_name: data.invitation.renter_name?.split(' ')[0] || prev.first_name,
                                last_name: data.invitation.renter_name?.split(' ').slice(1).join(' ') || prev.last_name,
                                email: data.invitation.renter_email || prev.email,
                            }))
                        }
                    }
                } catch (err) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (!resolvedAppId && supabase) {
                const insertData: any = { // eslint-disable-line @typescript-eslint/no-explicit-any
                    tenant_data: { ...formData, documents: uploadedDocs },
                    status: 'SUBMITTED',
                    documents: uploadedDocs,
                }
                // Link to broker/property if coming from an invitation
                if (inviteData?.invitation) {
                    insertData.broker_id = inviteData.invitation.broker_id
                    insertData.property_id = inviteData.property?.id || invitePropertyId
                    insertData.invitation_id = inviteData.invitation.id
                }
            ...
            const res = await fetch('/api/apply/submit', {
                ...
                body: JSON.stringify({
                    ...
                    ownerEmail,
                    broker_id: inviteData?.invitation?.broker_id || undefined,
                    property_id: inviteData?.property?.id || invitePropertyId || undefined,
                    invitation_id: inviteData?.invitation?.id || undefined,
                        ...
                        {inviteData && (
                            <div className="mb-4 p-4 rounded-xl bg-emerald-500/10 border border-emerald-500/20">
                                <p className="text-sm font-medium text-emerald-400">
                                    {inviteData.invitation?.broker_name || 'Your broker'} invited you to apply for rental protection
                                </p>
                                <p className="text-xs text-emerald-400/70 mt-1">
                                    {inviteData.property?.address}, {inviteData.property?.city}, {inviteData.property?.state} &mdash; ${Number(inviteData.property?.monthly_rent || 0).toLocaleString()}/mo
                                </p>
                            </div>
                        )}
                        {appId && !inviteData && (
